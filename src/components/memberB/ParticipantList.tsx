  ...
  return (
    <div className="bg-slate-50 p-5 rounded-xl border border-slate-200/60 shadow-sm">
      <h3 className="font-bold text-slate-800 mb-3 text-base">
        👤 투표 완료 인원 ({participants.length}명)
      </h3>
      <div className="flex flex-wrap gap-2">
        {participants.map((p) => {
          ...
          return (
            <div
              key={p.name}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border shadow-sm transition-all ${
                isMe
            ...
            >
              ...
              {isMe && (
                <div className="flex items-center gap-1 ml-1 border-l border-blue-200 pl-1.5">
                  ...
                  <button
                    onClick={() => {
                      if (confirm("내 일정을 정말 삭제하시겠습니까?"))
                        onDeleteParticipant(p.name);
                    }}
